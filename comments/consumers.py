...
class PostCommentsConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        self.comment_model = apps.get_model(
            app_label="comments", model_name=f"comment{self.model_type}"
        )

        if self.model_type == "post":
            self.model = Post
        else:
            self.model = apps.get_model(app_label="titles", model_name=self.model_type)
        self.r = redis.Redis(
    ...
    async def receive(self, text_data: Union[str, bytes]):
        ...
        await self.channel_layer.group_send(
            ...
            {"type": text_json_load["type"], "message": new_comment},
        )

    async def increment_views(self, event):
        message = event["message"]
        await self.send(
            text_data=json.dumps({"type": "increment_views", "message": message})
        )

    async def new_comment(self, event):
        message = event["message"]
        await self.send(
            text_data=json.dumps({"type": "new_comment", "message": message})
        )

    async def delete_comment(self, event):
        message = event["message"]
        await self.send(
            text_data=json.dumps({"type": "delete_comment", "message": message})
        )
    ...
    def _new_comment(self, data: Dict[str, str]) -> Dict[str, Union[str, int]]:
        ...
        try:
            parent = (
                self.comment_model.objects.get(id=get_parent) if get_parent else None
            )
