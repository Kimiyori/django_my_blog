...
class MangaTests(TestCase):
    ...
    def setUp(self):
        ...
        self.title2 = Title.objects.create(
            original_name='Tokyo Ghoul', russian_name='второй тест', english_name='second test')

        self.manga_type = MangaType.objects.create(
            name='manga', slug=slugify('test'))
    ...
    def test_anime_detail(self):
        response = self.client.get(
            reverse('anime_detail', args=[self.anime.pk]))
    ...
    def test_manga_detail(self):
        response = self.client.get(
            reverse('manga_detail', args=[self.manga.pk]))
