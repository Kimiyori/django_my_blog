...
def add_score(id: UUID, type: str) -> Optional[int]:
    ...
    model: Type[Anime | Manga] = apps.get_model(app_label="titles", model_name=type)
        ...
        if getattr(instance, "urls") and getattr(instance.urls, "mal"):
            try:
                mal_id: str = re.findall(
                    f"https://myanimelist.net/{type}/(\d+)/", instance.urls.mal
                )[0]
            except IndexError:
                file_logger.debug(
                    f"""Error due to the fact that the link from the {type} instance 
            ...
            if type == "anime":
                score: int = api.anime(int(mal_id)).GET(fields="mean").get("mean", 0)
            elif type == "manga":
                score: int = api.manga(int(mal_id)).GET(fields="mean").get("mean", 0)
            ...
            file_logger.debug(
                f"""{type.capitalize()} instance with following id {id} does not have 
...
def update_scores(type: str) -> NoReturn:
    ...
    for title in model.objects.all():
        if getattr(title, "urls") and getattr(title.urls, "mal"):
            try:
                mal_id: str = re.findall(
                    f"https://myanimelist.net/{type}/(\d+)/", title.urls.mal
                )[0]
            except IndexError:
                file_logger.debug(
                    f"""Error due to the fact that the link from the {type} instance 
                ...
                continue
            score: int = (
                getattr(title, "score", 0) if getattr(title, "score", 0) != None else 0
            )
            if type == "anime":
                upd_score: int = (
                    api.anime(int(mal_id)).GET(fields="mean").get("mean", 0)
                )
            elif type == "manga":
                upd_score: int = (
                    api.manga(int(mal_id)).GET(fields="mean").get("mean", 0)
                )
            ...
            file_logger.debug(
                f"{type.capitalize()} with id {title.id} doesnt have either urls or urls.mal"
            )
