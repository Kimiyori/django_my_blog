...
class SetScoreTaskTestCase(object):
    def setUp(self) -> None:
        ...
        self.modeltype = apps.get_model(app_label="titles", model_name=self.model)

        self.item = self.modeltype.objects.create(title=self.title, urls=self.urls)
    ...
    def test_success(self):
        ...
        self.assertEqual(
            self.item.score, Decimal(task.get()).quantize(Decimal(10) ** -2)
        )
    ...
    def test_wrong_url(self):
        ...
        item = self.modeltype.objects.create(title=self.title, urls=urls)
...
class UpdateScoresTaskTestCase(object):
    def setUp(self) -> None:
        ...
        self.item1 = self.modeltype.objects.create(title=self.title, urls=self.urls1)
        self.item2 = self.modeltype.objects.create(title=self.title, urls=self.urls2)
    ...
    def test_success(self):
        ...
        self.assertEqual(
            self.item1.score, Decimal(task.get()[0].score).quantize(Decimal(10) ** -2)
        )
        self.assertEqual(
            self.item2.score, Decimal(task.get()[1].score).quantize(Decimal(10) ** -2)
        )
    ...
    def test_wrong_url(self):
        ...
        self.item3 = self.modeltype.objects.create(title=self.title, urls=self.urls3)
        task = update_scores.delay(type=self.model)
...
class AnimeUpdateScoresTaskTestCase(UpdateScoresTaskTestCase, TransactionTestCase):
    def setUp(self) -> None:
        self.model = "anime"
        self.list_right_urls: list[str] = [
            "https://myanimelist.net/anime/11433/",
...
class MangaUpdateScoresTaskTestCase(UpdateScoresTaskTestCase, TransactionTestCase):
    def setUp(self) -> None:
        self.model = "manga"
        self.list_right_urls: list[str] = [
            "https://myanimelist.net/manga/121727/",
