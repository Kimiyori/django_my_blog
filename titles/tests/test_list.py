...
class BaseTitleListTests(object):
    ...
    def test_homepage_url_resolves_homepageview(self):
        ...
        self.assertEqual(view.func.__name__, TitleList.as_view().__name__)

    def test_list_items(self):
        values = ["id", "title__original_name", "image__thumbnail"]
        self.assertQuerysetEqual(
            self.response.context["list"],
            self.modeltype.objects.all()
            .values(*values)
            .order_by("title__original_name"),
        )

    def test_no_filter_name(self):
        q = ""
        url_filter_name = self.get_url(self.url, urlparams(q=q))
    ...
    def test_filter_attributes(self):
        ...
        url_filter_attr = self.get_url(self.url, urlparams(genre=self.genre2))
