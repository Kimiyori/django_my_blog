...
class Filter:
    ...
    def __call__(self, name: str, item: str) -> Dict[str, str]:
        return {f"{name}__name__icontains": item}


def filter_by_name(
    request: HttpRequest, item: QuerySet[Union[Manga, Anime]]
...
def annotate_acc(type: str, tab: str) -> Dict[str, ArrayAgg]:
    ...
    def create_array(list: list[str]) -> ArrayAgg:
        ...
        # if values more that 1, then use psql Array func to wrap it
            ...
            arr = Array(
                *[create_wrapper(name) for name in list],
        ...
        "manga": {
            "info": {
                ...
                "related_posts": [
                    "related_post__id",
                    "related_post__title",
                    "related_post__main_image",
                ],
            },
            "related": {
                "adaptations": [
                    "adaptation__adaptation__id",
                    "adaptation__adaptation__image__thumbnail",
                    "adaptation__adaptation__title__original_name",
                    "anime",
                ],
                "based_ons": [
                    "based_on__based_on__id",
                    "based_on__based_on__image__thumbnail",
                    "based_on__based_on__title__original_name",
                    "anime",
                ],
                "sequels": [
                    "sequel__sequel__id",
                    "sequel__sequel__image__thumbnail",
                    "sequel__sequel__title__original_name",
                ],
                "prequels": [
                    "prequel__prequel__id",
                    "prequel__prequel__image__thumbnail",
    ...
    annotate_dict = {
        key: create_array(value) for (key, value) in d_anno[type][tab].items()
    }
...
def filter_by_models(
    request: HttpRequest, instance: QuerySet[Union[Manga, Anime]]
...
def get_comments(type: str, id: int) -> Type[Union[CommentManga, CommentAnime | None]]:
    ...
    """
    return (
        apps.get_model(app_label="comments", model_name=f"comment{type}")
        .objects.select_related("author__profile")
        .filter(model=id)
    )
...
def get_instance(model, type, tab, id):
    return (
        model.objects.annotate(**annotate_acc(type, tab))
