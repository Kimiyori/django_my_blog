...
class MyAnimeList:
    MYANIMELIST_URL: str = 'https://myanimelist.net'
    _TOKEN_URL = MYANIMELIST_URL + '/v1/oauth2/token'
    ...
    def get_auth_url(self) -> str:
        auth_url = self.MYANIMELIST_URL + '/v1/oauth2/authorize'
        return self._client.authorization_url(auth_url, code_challenge=self.code_challenge)[0]

    def fetch_token(self, code: str) -> dict:
        self._client.fetch_token(self._TOKEN_URL, code=code, code_verifier=self.code_verifier,
...
class ApiMethod:
    ...
    def __init__(self, session: MyAnimeList, path: str = '') -> None:
        self._session = session
        self._path = path

    def __call__(self, item) -> ApiMethod:
        # item: SupportsStr
