...
class TitleList(ListView):
    ...
    def get_queryset(self) -> QuerySet[Union[Manga, Anime]]:
        ...
        model: Type[Anime | Manga] = apps.get_model(
            app_label="titles", model_name=self.type
        ).objects.all()
        ...
        model = model.values("id", "title__original_name", "image__thumbnail").order_by(
            "title__original_name"
        )
        console_logger.info(
            f"Successfil get list of {self.type}",
            extra={"filers": params, "title_filter": title_name},
        )
        return model
    ...
    def get_context_data(self, **kwargs: Any) -> HttpResponse:
        ...
        # update context
        context.update(
            {"filter": filter_dict, "model": self.type.capitalize(), "query": query}
        )
...
class TitleDetail(TemplateResponseMixin, View):
    ...
    def get(self, request: HttpRequest, pk: uuid4) -> HttpResponse:
        ...
        modeltype: Type[Anime | Manga] = apps.get_model(
            app_label="titles", model_name=type
        )
