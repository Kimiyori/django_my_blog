...
class TestTitle(APITestCase):
    ...
    def test_create_full_serializer(self):
        ...
        self.assertEqual(created.original_name, self.serializer_data["original_name"])
    ...
    def test_create_partial_serializer(self):
        ...
        self.assertTrue(serializer.is_valid())
        created = serializer.save()
        self.assertEqual(created.original_name, serializer_data["original_name"])
    ...
    def test_update_serializer(self):
        ...
        self.assertTrue(serializer.is_valid())
        obj = serializer.save()
        self.assertEqual(obj.original_name, self.serializer_data["original_name"])
        updates = serializer.update(
            instance=obj, validated_data={"original_name": "test_updated"}
        )
...
class TestAuthors(APITestCase):
    def setUp(self):
        ...
        self.author_data = {"name": "test1", "photo": get_temporary_image()}
        self.artist_data = {"name": "test1", "photo": get_temporary_image()}
...
class TestTitlesSerializator:
    ...
    def test_create(self):
        ...
        self.assertEqual(model.title.original_name, self.data["title"]["original_name"])
        self.assertEqual(model.title.english_name, self.data["title"]["english_name"])
    ...
    def test_update(self):
        ...
        self.assertTrue(serializer.is_valid())
        model = serializer.save()
        update_serializer = self.serializer(
            instance=model,
            data={
                "title": {"english_name": "test_update"},
                "genre": ["Drama"],
                "theme": ["Monsters"],
            },
        )
        ...
        self.assertEqual(
            list(model.genre.all().values_list("name", flat=True)), ["Comedy", "Drama"]
        )
        self.assertEqual(
            list(model.theme.all().values_list("name", flat=True)),
...
class TestAnime(TestTitlesSerializator, APITestCase):
    fixtures = [
        "titles/fixtures/fixtures_animetype.yaml",
        "titles/fixtures/fixtures_genre.yaml",
        "titles/fixtures/fixtures_studio.yaml",
        "titles/fixtures/fixtures_theme.yaml",
    ]
...
class TestManga(TestTitlesSerializator, APITestCase):
    fixtures = [
        "titles/fixtures/fixtures_mangatype.yaml",
        "titles/fixtures/fixtures_genre.yaml",
        "titles/fixtures/fixtures_magazine.yaml",
        "titles/fixtures/fixtures_theme.yaml",
        "titles/fixtures/fixtures_publisher.yaml",
        "titles/fixtures/fixtures_demographic.yaml",
    ]
    ...
    def setUp(self):
        ...
        self.data = {
            ...
            "volumes": 12,
            "chapters": 96,
            "authors": {"author": {"name": "string"}, "artist": {"name": "string"}},
            "publisher": ["Shueisha"],
            "genre": [
                "Comedy",
            ],
...
class TestPostSerializer(APITestCase):
    fixtures = [
        "post/fixtures/fixtures_post.yaml",
