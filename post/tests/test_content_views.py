...
class PostModelTests(TestCase):
    ...
    def test_create_post_post(self):
        ...
        img = image = get_temporary_image("text image for create url", height=456)
        response = self.client.post(
            reverse("post_detail_create"), data={"title": "test", "main_image": img}
        )
    ...
    def test_post_delete_fail(self):
        self.client.force_login(self.user)
        response = self.client.post(reverse("post_delete", args=[uuid4()]))
    ...
    def test_post_detail_change_get(self):
        ...
        self.assertTemplateUsed(response_get, "post/manage/content/form.html")
    ...
    def test_post_detail_change_wrong_id(self):
        self.client.force_login(self.user)
        response_get = self.client.get(reverse("post_detail_change", args=[uuid4()]))
        ...
        with self.assertRaises(ValidationError, msg="validation error"):
            response = self.client.post(
                reverse(
                    ...
                    kwargs={"post_id": self.post.pk, "model_name": "video", "order": 2},
                ),
                data={"text": "hello"},
            )
    ...
    def test_modules_create_text_without_order(self):
        ...
        self.assertEqual(
            Content.objects.get(post=self.post.pk, order=5).item.text, "hello"
        )
    ...
    def test_modules_create_text(self):
        ...
        response = self.client.post(
            reverse(
                "module_content_create",
                kwargs={"post_id": self.post.pk, "model_name": "text", "order": 2},
            ),
            data={"text": "hello"},
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            Content.objects.get(post=self.post, order=3).item.text, "hello"
        )
        self.content_image.refresh_from_db()
    ...
    def test_modules_create_image(self):
        self.client.force_login(self.user)
        self.assertEqual(
            Content.objects.get(post=self.post, order=3).item.image.height, 1000
        )
        image = get_temporary_image("text image for create url", height=456)
        response = self.client.post(
            reverse(
                "module_content_create",
                kwargs={"post_id": self.post.pk, "model_name": "image", "order": 2},
            ),
            data={"image": image},
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            Content.objects.get(post=self.post, order=3).item.image.height, 456
        )
        self.assertEqual(
            Content.objects.get(post=self.post, order=4).item.image.height, 1000
        )

    def test_modules_create_video_url(self):
        self.client.force_login(self.user)
        self.assertEqual(
            Content.objects.get(post=self.post, order=1).item.text, "Some tests"
        )
        url = "https://www.youtube.com/watch?v=U6IPdxz24ao&list=RDU6IPdxz24ao&start_radio=1&ab_channel=EpicAnimeMusic"
        response = self.client.post(
            reverse(
                "module_content_create",
                kwargs={"post_id": self.post.pk, "model_name": "video", "order": 0},
            ),
            data={"video": url},
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            Content.objects.get(post=self.post, order=2).item.text, "Some tests"
        )
        self.assertEqual(
            Content.objects.get(post=self.post, order=1).item.video,
            YoutubeBackend(url=url).get_url(),
        )

    def test_model_create_file(self):
        self.client.force_login(self.user)
        new_file = SimpleUploadedFile(
            "new_test_update.txt", b"these are the file contents!"
        )
        response_file = self.client.post(
            reverse(
                "module_content_create",
                kwargs={"post_id": self.post.pk, "model_name": "file", "order": 1},
            ),
            data={"file": new_file},
        )
        self.assertEqual(response_file.status_code, 200)
        self.assertEqual(
            Content.objects.get(post=self.post, order=2).item.file.name.split("/")[-1],
            new_file.name,
        )
    ...
    def test_model_update_image(self):
        self.client.force_login(self.user)
        response_image = self.client.post(
            reverse(
                "module_content_update",
                kwargs={
                    "post_id": self.post.pk,
        ...
        self.assertEqual(
            Content.objects.get(post=self.post, order=3).item.image.height, 789
        )

    def test_model_update_video(self):
        self.client.force_login(self.user)
        self.assertEqual(
            Content.objects.get(post=self.post, order=4).item.video,
            YoutubeBackend(
                url="https://youtu.be/xahEdP2eJs4?list=RDxahEdP2eJs4"
            ).get_url(),
        )
        ...
        self.assertEqual(
            Content.objects.get(post=self.post, order=4).item.video,
            YoutubeBackend(url=video).get_url(),
        )

    def test_model_update_file(self):
        self.client.force_login(self.user)
        self.assertEqual(
            Content.objects.get(post=self.post, order=2).item.file, self.file_obj.file
        )
        new_file = SimpleUploadedFile(
            "new_test_update.txt", b"these are the file contents!"
        )
    ...
    def test_content_delete__success(self):
        ...
        response = self.client.post(
            reverse(
                "content_delete",
                kwargs={"post_id": str(self.post.id), "id": self.content_text.id},
            )
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(Content.objects.count(), 3)
        self.assertFalse(Content.objects.filter(post=self.post, id=content_id).exists())
    ...
    def test_content_delete_fail_post_id(self):
        ...
        response = self.client.post(
            reverse(
                "content_delete",
                kwargs={"post_id": str(uuid4()), "id": self.content_text.id},
            )
        )
        self.assertEqual(response.status_code, 404)
    ...
    def test_content_delete_fail_content_id(self):
        ...
        response = self.client.post(
            reverse(
                "content_delete", kwargs={"post_id": str(self.post.id), "id": 1212121}
            )
        )
        self.assertEqual(response.status_code, 404)
    ...
    def test_swap_orders(self):
        self.client.force_login(self.user)
        data = {
            str(self.content_text.id): str(self.content_image.order),
